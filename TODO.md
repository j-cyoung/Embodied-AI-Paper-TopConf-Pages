...
## 当前优先级 TODO（2026-02-11）

### P0 - 可用性与可诊断性
- [ ] API Key 来源诊断日志增强（明确输出 `prefs/env/none`）
- [ ] 当 API Key 缺失时，弹出可操作提示（含修复建议，而不仅是日志）
- [ ] 查询入口统一错误处理：右键菜单、Tools 菜单都显示一致错误提示

### P1 - 环境变量兼容性
- [ ] macOS 启动方式适配说明（Dock/Finder 启动不继承 shell 环境）
- [ ] 增加“环境变量可见性自检”入口（例如 Settings 中一个 `Check Runtime` 按钮）
- [ ] 若读取到环境变量，显示简要提示（仅来源，不泄露密钥）

### P1 - 设置页体验
- [ ] Settings 新增“测试连接”按钮（校验 base_url/model/key）
- [ ] 对关键字段增加输入校验与即时提示（如 URL、并发、token 范围）
- [ ] 增加“保存后需重启服务”的自动引导按钮（可一键重启服务）

### P2 - 可靠性与回归
- [ ] 补充插件侧集成测试：缺 key、无服务、服务超时、HTTP 4xx/5xx
